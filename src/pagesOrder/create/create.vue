...
import type { OrderPreGoods, OrderSummary, OrderPreResult } from '@/types/order'
import type { AddressItem } from '@/types/goods'
import { useAddressrStore } from '@/stores/modules/address'
...
// 商品信息
const goodsList = ref<OrderPreGoods[]>([])
// 结算信息
const summaryData = ref<OrderSummary>()
// 地址信息
...
const getMemberOrderPreData = async () => {
  try {
    ...
    const { goods, summary, userAddresses } = result
    goodsList.value = goods
    summaryData.value = summary
    addressList.value = userAddresses
...
// 计算默认收获地址
const addressStore = useAddressrStore()
...
const onOrderSubmit = async () => {
  try {
    ...
    const res = await postMemberOrder({
      ...
      goods: goodsList.value.map(({ count, skuId }) => ({ count, skuId })),
      payChannel: 2,
      payType: 1,
    })
    // 关闭当前页面，跳转到订单详情页面
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="settlement">
      <view class="item">
        ...
        <text class="number symbol">{{ summaryData?.totalPrice.toFixed(2) }}</text>
      </view>
      <view class="item">
        <text class="text">运费: </text>
        <text class="number symbol">{{ summaryData?.postFee.toFixed(2) }}</text>
      </view>
    </view>
  </scroll-view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="total-pay symbol">
      <text class="number">{{ summaryData?.totalPayPrice.toFixed(2) }}</text>
    </view>
