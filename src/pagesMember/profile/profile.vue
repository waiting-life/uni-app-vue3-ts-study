...
import { getMemberProfile, putMemberProfileAPI } from '@/services/profile'
...
// 会员信息
const profileDetail = ref<ProfileDetail>({} as ProfileDetail)
const getMemberProfileDetail = async () => {
  try {
    const res = await getMemberProfile()
    // console.log(res, 'profileDetail')
      ...
      uni.uploadFile({
        ...
        success: (res) => {
          if (res.statusCode === 200) {
            ...
            // 当前页面更新头像
            profileDetail.value!.avatar = avatar
            // 更新 Store 头像
...
const onSubmit = async () => {
  const { nickname, gender, birthday } = profileDetail.value
...
const onGenderChange: UniHelper.RadioGroupOnChange = (event) => {
  profileDetail.value.gender = event.detail.value as Gender
}
// 修改生日
const onBirthdayChange: UniHelper.DatePickerOnChange = (event) => {
  console.log(event.detail.value)
  profileDetail.value.birthday = event.detail.value
}
// 修改城市
...
const onLocationChange: RegionPickerOnChange = (event) => {
  ...
  // 修改前端界面
  profileDetail.value.fullLocation = value.join(' ')
  // 提交后段更新
